...
import { Clock, CheckCircle2, ChevronDown, Trophy } from "lucide-react";
...
export function ProjectStatusCard({
  ...
  return (
    <Card
      className="group w-full cursor-pointer overflow-hidden border border-gray-200 bg-white shadow-sm transition-all duration-300 hover:shadow-lg hover:border-teal-500/30 rounded-xl"
      ...
      <div className="p-6">
        <div className="flex justify-between items-start mb-4">
          ...
          <div className="flex items-center justify-center w-10 h-10 rounded-full bg-teal-50 text-teal-600 group-hover:bg-teal-600 group-hover:text-white transition-colors duration-300">
            {progress === 100 ? <Trophy size={20} /> : <Clock size={20} />}
          </div>
        </div>
            ...
            <div className="p-5 space-y-4">
              ...
              <ul className="space-y-3">
                {tasks.map((task, index) => (
                  ...
                  >
                    <div className="mt-0.5 mr-3 flex-shrink-0 w-1.5 h-1.5 rounded-full bg-teal-400" />
